...
#? Display Group/Channel Joining Prompt
async def join_prompt(update):
    JoinButton = [
        [InlineKeyboardButton("Join Group↗",  url='https://t.me/+LZrrLPWvf4djM2Y1')],
        [InlineKeyboardButton("Join Channel↗",  url='https://t.me/+5lKi2mcQGoNjNzJl')],
        [InlineKeyboardButton("Check✅", callback_data=str(ONE))]
    ]
    text=f'''Hello!, Welcome to <a href='https://chatai.typedream.app/'>ChatAI</a> 😊

It looks like you haven't join our group and channel yet.
Please make sure you join both in order to use this bot!
'''
    try:
...
#? Check if user is the member of the Group and Channel
async def is_user_member (chat_id):
    with open('group_users.json', 'r') as f:
        chat_ids = json.loads(f.read())
    is_in_group = chat_id in chat_ids
    
    with open('channel_users.json', 'r') as f:
        chat_ids = json.loads(f.read())
    is_in_channel = chat_id in chat_ids
...
#? Wrapper for Group/Channel Control
async def check_member (update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    group_status = await is_user_member(chat_id)
    if group_status:
        if x: return await BotOptionsCallApart(update)
        else: return await BotOptions (update, context)
    else:
        return await join_prompt(update)
...
async def grp_req_reject (update):
    #* Add as a new user (if user already in the database nothing will change)
    status = await new_user(update)
    await update.message.reply_text(
            text = '''
🚫 You have not joined either the group or channel yet
 
Group: <a href='https://t.me/+LZrrLPWvf4djM2Y1'>GROUP ↗</a>
Channel: <a href='https://t.me/+5lKi2mcQGoNjNzJl'>CHANNEL ↗</a>
''',
            disable_web_page_preview=True,
            parse_mode='HTML'
    )
...
async def group_req_text (update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    group_status = await is_user_member(user_id)
...
#??????????????????????????????????????????????????????????????????????????????????????????????????
#? This version will allow you to use bot in groups.
#? Allows you to Force user to join admin's group and channel
#??????????????????????????????????????????????????????????????????????????????????????????????????
##******************************************************* MAIN FUNCTION ******************************************************
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            INIT_GROUP: [CallbackQueryHandler(check_member, pattern='^' + str(ONE) + '$')],
            MAIN: [
        ...
        # run_async=True,
        fallbacks=[
            CommandHandler(['start', 'menu'], check_member),
            MessageHandler(filters.TEXT & ~filters.COMMAND, check_member)
        ]
    )
    ...
    #* Admin Group/Channel Member Update
