...
async def join_prompt(context: ContextTypes.DEFAULT_TYPE, chat_id):
    JoinButton = [
...
async def check_member (update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if group_status:
        ...
        return await join_prompt(context, chat_id)
...
from group_handler_bot import update_chat_members
#??????????????????????????????????????????????????????????????????????????????????????????????????
#? This version allows you to Force user to join admin's group and channel
#??????????????????????????????????????????????????????????????????????????????????????????????????
##******************************************************* MAIN FUNCTION ******************************************************
def main():
